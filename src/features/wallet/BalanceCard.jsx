...
const BalanceCard = ({ activeNetwork, onNetworkChange, onSend, onReceive, onSwap, onScan }) => {
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-6 lg:gap-8 min-h-[80vh]">
      {/* Wallet Card */}
      <div className="flex-1 min-w-0 bg-[#0C120F] p-4 pt-5 sm:p-6 md:p-10 rounded-3xl md:rounded-[32px] flex justify-center border border-white/5 relative overflow-hidden shadow-2xl">
        {/* Background Effects */}
        ...
        <div className="relative z-10">
          ...
          {/* Balance Section */}
          <div className="text-center sm:text-left mb-8 sm:mb-6 mt-10 sm:mt-15 md:mb-8 flex flex-col justify-center items-center">
            ...
            <div className="flex items-baseline justify-center gap-2 animate-fade-in-up">
              <h1 className="text-5xl sm:text-6xl md:text-7xl font-black text-white tracking-tighter leading-none">{totalUsd}</h1>
              <span className="text-sm sm:text-xl font-bold text-[#8FA396]/40">USD</span>
            </div>
          </div>
      ...
      {/* Stats/Poster Card */}
      <div className="w-full lg:w-[420px] xl:w-[480px] shrink-0 rounded-3xl md:rounded-[32px] border border-white/5 relative overflow-hidden shadow-2xl flex flex-col gap-4 md:gap-6">
        {/* Background Effects */}
        <TokenList />
